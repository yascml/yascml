/**
 * Infinite-arity SemVer implementation, faithful port of the SC2ML upstream
 * `InfiniteSemVer.ts`. Supports arbitrarily long version numbers (e.g. `1.2.3.4`),
 * `>`/`>=`/`<`/`<=`/`=`/`^` operators, pre-release / build-metadata suffixes, and
 * `||` (OR of sets) / `&&` (AND within a set) ranges.
 *
...
const isCharNumber = (c: string): boolean => (
  typeof c === 'string' && c.length === 1 && c >= '0' && c <= '9'
);

export const parseInfiniteSemVer = (versionStr: string): VersionBoundary => {
  versionStr = versionStr.trim();
  let bo: BoundaryOperator = undefined;

  const bb = BoundaryOperatorList
    .filter((T): T is Exclude<BoundaryOperator, undefined> => !!T)
  ...
  if (versionStr.length === 0 || !isCharNumber(versionStr[0])) {
    ...
    return {
      version: { version: [], preRelease: undefined, buildMetadata: undefined },
      operator: bo,
    };
  }
...
export const compareString = (a: string, b: string): number => a.localeCompare(b);
...
const parseVersionBoundarySet = (setStr: string): VersionBoundarySet | null => {
...
export const parseVersionRange = (rangeStr: string): VersionRange => {
  const rangeSets = rangeStr
    .split(/\s*\|\|\s*/)
...
export const isWithinRange = (version: InfiniteSemVer, range: VersionRange, ignorePostfix = false): boolean => {
